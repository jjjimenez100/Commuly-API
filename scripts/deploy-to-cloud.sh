...
# -c url to use to fetch rsa keys for Cloudfront
...
while getopts p:s:c:u:d:b:e: option
do
case "${option}"
in
p) SSH_KEYS_URL=${OPTARG};;
s) SSH_KEYS=${OPTARG};;
c) CLOUDFRONT_URL=${OPTARG};;
u) HOST_URL=${OPTARG};;
...
ssh -i ${SSH_KEYS} ${HOST_URL} -o StrictHostKeyChecking=no "cd ${PROJECT_DIRECTORY} && git pull origin ${BRANCH_NAME} && wget ${ENV_URL} -O .env && wget ${CLOUDFRONT_URL} -O commuly-api-cloudfront-key.pem && yarn install && pm2 restart all"
